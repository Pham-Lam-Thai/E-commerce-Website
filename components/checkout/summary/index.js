...
export default function Summary({totalAfterDiscount, setTotalAfterDiscount, user,cart,paymentMethod,selectedAddress}) {
    const [coupon, setCoupon]= useState("");
    const [discount, setDiscount]= useState("");
    const [error, setError]= useState("");
    const [order_error, setOrder_Error]= useState("");
    const validateCoupon = Yup.object({
        coupon: Yup.string().required("Please enter the coupon first!"),
    });
    ...
    const placeOrderHandler = async () => {
        try {
            if(paymentMethod == ""){
                setOrder_Error("Please choose a payment.");
                return;
            }else if(!selectedAddress){
                setOrder_Error("Please choose shipping address.");
                return;
            }
