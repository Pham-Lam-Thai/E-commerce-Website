...
export default function TableSelect({ property, text, data, handleChange}) {
  ...
  return (
    <div className={styles.select}>
        ...
        >
            <span 
            className={`${styles.flex} ${styles.select_header_wrap}`}
            style={{ padding: "0 5px", 
            }}
            >
            ...
            {visible && (
                <ul className={styles.select_header_menu}
                onMouseOver={()=>setVisible(true)}
                onMouseLeave={()=>setVisible(false)}
                    ...
                    {data.map((item, i) => {
                        ...
                        if(text == "Style"){
                            return (
                                <li 
                                    ...
                                    <span>
                                        {
                                            item.image ? <img src={item.image} alt=""/> : "All Colors"
                                        }
                                    </span>
                                </li>
                            );
                        ...
                        if(text == "Order"){
                            return (
                                <li 
                                    style={{width:text == "Order " && "200px"}}
                                    key={i} onClick={() => handleChange(item.value)}>
                                    <span>{item.text}</span>
                                </li>
                            );
                        }
